...
void VulkanCommandBuffer::Create(IDevice* a_device , ISwapChain* a_swapChain, ICommandPool* a_commandPool) {
	...
	l_allocateInfo.commandBufferCount = (uint32_t)a_swapChain->CastVulkan()->GetCommandBuffers().size();

	if (vkAllocateCommandBuffers(a_device->CastVulkan()->GetDevice(), &l_allocateInfo, a_swapChain->CastVulkan()->GetCommandBuffers().data()) != VK_SUCCESS) {
