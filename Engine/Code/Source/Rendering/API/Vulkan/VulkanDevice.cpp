...
void VulkanDevice::CreateLogicalDevice(const VkSurfaceKHR a_surface, VkInstance a_instance)
{
	...
	for (int l_queueFamilyIndex : l_queueFamilyIndices) {
		...
		l_queueCreateInfo.pQueuePriorities = &l_priority; // vulkan need to know how to handle multiple queues, so decide property ( 1 = highest prioryty)
	...
	VkResult result = vkCreateDevice(m_physicalDevice, &l_deviceCreateInfo, nullptr, &m_device);
...
SwapChainDetails VulkanDevice::GetSwapChainDetails(VkPhysicalDevice a_device , VkSurfaceKHR a_surface)
{
...
bool VulkanDevice::CheckDeviceExtensionSupport(VkPhysicalDevice a_device)
{
	...
	for (const auto& l_deviceExtension : deviceExtensions)
	{
		...
		{
			if (strcmp(l_deviceExtension, extension.extensionName) == 0)
			{
				hasExtension = true;
				break;
			}
...
QueueFamilyIndices VulkanDevice::GetQueueFamilies(VkPhysicalDevice a_device, VkSurfaceKHR a_surface)
{
