...
void VulkanSynchronization::Create(IDevice* a_device)
{
    ...
    l_fenceInfo.pNext = nullptr;

	for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
	{
		if (vkCreateSemaphore(a_device->CastVulkan()->GetDevice(), &l_semaphoreInfo, nullptr, &m_imageAvailableSemaphores[i]) != VK_SUCCESS || vkCreateSemaphore(a_device->CastVulkan()->GetDevice(), &l_semaphoreInfo, nullptr, &m_renderFinishedSemaphores[i]) != VK_SUCCESS || vkCreateFence(a_device->CastVulkan()->GetDevice(), &l_fenceInfo, nullptr, &m_fences[i]) != VK_SUCCESS)
